...
// ─── Types matching the real MTC API ───────────────────────────────────────────
...
export default function App() {
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // 1. Load data from MTC API on mount and auto-refresh every 6 seconds
  // ─────────────────────────────────────────────────────────────────────────────
  useEffect(() => {
    let mounted = true;
    ...
    // Refresh fleet list periodically
  ...
  const simulationStep = useCallback(async () => {
    ...
    // POST ETM feed
    try {
      const etmRes = await apiFetch("/feeds/etm", {
        method: "POST",
        body: JSON.stringify({
  ...
  const injectSecurityAlert = useCallback(async () => {
    ...
    try {
      ...
      addLog("SECURITY", "INFO", "Alert dispatched to MTC Operator Console (`http://localhost:3000`)");
    } catch {
  ...
  return (
    <div className="min-h-screen bg-[#0b151c] text-slate-100 flex flex-col font-sans selection:bg-amber-500 selection:text-slate-950">

      {/* ── TOP HEADER (Matching MTC SmartBus Port 3000 theme) ──────────────────── */}
      <header className="bg-[#0f1b24]/90 border-b border-slate-800/80 sticky top-0 z-50 backdrop-blur-md px-4 lg:px-8 py-3.5 flex flex-wrap items-center justify-between gap-4 shadow-lg shadow-black/20">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="font-heading font-bold text-lg leading-none tracking-tight text-white">
            ...
            <p className="text-[11px] text-slate-400 font-mono mt-0.5 flex items-center gap-1.5">
              ...
              <span className="text-emerald-400 font-medium">Syncs with Main App (Port 3000)</span>
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-heading font-bold flex items-center gap-2 transition-all shadow-md disabled:opacity-40 disabled:cursor-not-allowed ${
              isPlaying
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="px-3 py-2 bg-teal-500/10 hover:bg-teal-500/20 text-teal-300 border border-teal-500/30 rounded-xl text-xs font-mono font-semibold flex items-center gap-1.5 transition-all"
            title="Open MTC SmartBus Main Dashboard on port 3000"
          >
            <span>Port 3000</span>
      ...
      {/* ── MAIN DASHBOARD GRID ─────────────────────────────────────────────────── */}
      <main className="flex-1 p-4 lg:p-6 grid grid-cols-1 lg:grid-cols-12 gap-5 max-w-[1700px] mx-auto w-full">

        {/* ── LEFT COLUMN: Live Fleet Selector (3 cols) ────────────────────────── */}
          ...
          <div className="bg-[#0f1b24] border border-slate-800 rounded-2xl p-4 shadow-xl flex-1 flex flex-col">
            ...
            {apiLoading ? (
              ...
              <div className="bg-rose-500/10 border border-rose-500/30 text-rose-300 p-4 rounded-xl text-xs font-mono">
                ...
                <span>{apiError}</span>
                <p className="mt-2 text-[10px] text-slate-400">Ensure main server is active on `http://localhost:3000`</p>
              </div>
            ) : (
              <div className="space-y-2.5 overflow-y-auto max-h-[calc(100vh-230px)] pr-1.5 flex-1">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="flex items-center gap-2">
                          <span className="font-heading font-bold text-amber-400 text-sm tracking-tight">
                            Bus {bus.number}
                          </span>
        ...
        {/* ── CENTER COLUMN: Route Progression & Telemetry Controls (5 cols) ───── */}
        <div className="lg:col-span-5 flex flex-col gap-5">

          {/* Active Bus Target Card */}
        ...
        {/* ── RIGHT COLUMN: Reconciliation & Telemetry Stream (4 cols) ─────────── */}
